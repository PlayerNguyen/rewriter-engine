...
function twMerge(...classes: (string | undefined | false | null)[]): string {
  ...
  for (const cls of classes) {
    ...
    for (const c of cls.split(/\s+/)) {
      ...
      const prefix =
        c.match(
          /^(bg|text|border|rounded|p|px|py|pt|pb|pl|pr|m|mx|my|mt|mb|ml|mr|w|h|min|max|gap|flex|grid|justify|items|self|order|font|leading|tracking|shadow|ring|outline|opacity|z|inset|top|right|bottom|left|overflow|whitespace|truncate)/,
        )?.[0] || c;
